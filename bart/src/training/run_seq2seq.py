...
def main():
    ...
    parser.add_argument("--dev_tgt_file", default=None, type=str,
                        help="The output data file name.")
    parser.add_argument("--bart_model", default="facebook/bart-base", type=str,
                        help="Bart pre-trained model selected in the list: bert-base-uncased, "
                             "facebook/bart-base,facebook/bart-large")
    parser.add_argument("--config_path", default=None, type=str,
                        help="Bert config file path.")
    parser.add_argument("--output_dir",
                        default="../../../output/bart_base",
                        type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--log_dir",
                        default="../../../log/bart_base",
                        type=str,
    ...
    parser.add_argument('--max_len_a', type=int, default=32,
    ...
    parser.add_argument("--mask_prob_eos", default=0, type=float,
                        help="Number of prediction is sometimes less than max_pred when sequence is short.")
    parser.add_argument('--max_pred', type=int, default=64,
    ...
    if args.do_train:
        ...
        fn_src = os.path.join(
            args.data_dir, args.train_src_file if args.train_src_file else 'commongen.train.src_new.txt')
        fn_tgt = os.path.join(
            args.data_dir, args.train_tgt_file if args.train_tgt_file else 'commongen.train.tgt.txt')

        train_dataset = seq2seq_loader.Seq2SeqDataset(
            fn_src, fn_tgt, args.train_batch_size, data_tokenizer,
    ...
    if args.do_eval:
        ...
        fn_src = os.path.join(
            args.data_dir, args.dev_src_file if args.dev_src_file else 'commongen.dev.src_new.txt')
        fn_tgt = os.path.join(
            args.data_dir, args.dev_tgt_file if args.dev_tgt_file else 'commongen.dev.tgt.txt')

        dev_dataset = seq2seq_loader.Seq2SeqDataset(
            fn_src, fn_tgt, args.eval_batch_size, data_tokenizer,
            args.max_seq_length, bi_uni_pipeline=bi_uni_pipeline)
        if args.local_rank == -1:
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        if recover_step:
            ...
            global_step = 0
        model = BartForConditionalGeneration.from_pretrained(args.bart_model, state_dict=model_recover)
    if args.local_rank == 0:
        dist.barrier()
    ...
    if args.do_train:
        ...
        logger.info("  Training size = %d", len(train_dataloader))
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs) + 1, desc="Epoch",
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Training", position=0, leave=True)):
                ...
                if args.pretraining_KG:
                    ...
                    input_ids, subword_mask, word_subword, decoder_input_ids, decoder_attention_mask, labels = batch
                    oracle_pos, oracle_weights, oracle_labels = None, None, None
                loss_output = model(input_ids, attention_mask=subword_mask,
                                    decoder_input_ids=decoder_input_ids,
                                    decoder_attention_mask=decoder_attention_mask, labels=labels,
                                    label_smoothing=False, do_no_pos=args.do_no_pos)
